...
public class DavisBaseBinaryFileExample {
	...
	static void initializeDataStore() {
		...
		try {
			...
			catalogDir.mkdir();
			String[] oldCatalogFiles;
			oldCatalogFiles = catalogDir.list();
			for (int i=0; i<oldCatalogFiles.length; i++) {
				File anOldFile = new File(catalogDir, oldCatalogFiles[i]); 
				anOldFile.delete();
			}
			...
			userDataDir.mkdir();
			String[] oldDataFiles;
			oldDataFiles = userDataDir.list();
			for (int i=0; i<oldDataFiles.length; i++) {
				File anOldFile = new File(userDataDir, oldDataFiles[i]); 
				anOldFile.delete();
			}
		}
